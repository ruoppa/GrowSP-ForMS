...
class CutPursuit_SPG : public CutPursuit<T>
{
    ...
    { //-----initialize the labelling for each components with kmeans------------------------------
        ...
        #ifdef OPENMP
		#pragma omp parallel for if (nb_comp >= (uint32_t) omp_get_num_threads()) schedule(dynamic) 
        #endif
        for (uint32_t ind_com = 0; ind_com < nb_comp; ind_com++)
        {
            ...
            #ifdef OPENMP
            #pragma omp parallel for if (nb_comp < (uint32_t) omp_get_num_threads()) shared(best_energy) schedule(static) 
            #endif
            ...
            for (uint32_t ite_kmeans = 0; ite_kmeans < this->parameter.kmeans_ite; ite_kmeans++)
            {
                ...
                #ifdef OPENMP
                #pragma omp parallel for if (nb_comp < (uint32_t) omp_get_num_threads()) shared(potential_label) schedule(static) 
                #endif
				for (uint32_t i_ver = 0;  i_ver < comp_size; i_ver++)
                {
