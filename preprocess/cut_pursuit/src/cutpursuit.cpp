...
PyObject * cutpursuit(const bpn::ndarray & obs, const bpn::ndarray & source, const bpn::ndarray & target,const bpn::ndarray & edge_weight,
                      float lambda, const int cutoff, const int spatial, float weight_decay, bool verbose)
{//read data and run the L0-cut pursuit partition algorithm
    srand(0);
    float cp_verbose = verbose ? 1.f : 0.f;
    ...
    if (spatial == 0)
    {
        CP::cut_pursuit<float>(n_ver, n_edg, n_obs, obs_data, source_data, target_data, edge_weight_data, &node_weight[0]
                 , solution.data(), in_component, components, lambda, (uint32_t)cutoff,  1.f, 4.f, weight_decay, cp_verbose);
    }
    else
    {
        CP::cut_pursuit<float>(n_ver, n_edg, n_obs, obs_data, source_data, target_data, edge_weight_data, &node_weight[0]
                 , solution.data(), in_component, components, lambda, (uint32_t)cutoff,  2.f, 4.f, weight_decay, cp_verbose);
    }
...
PyObject * cutpursuit_hierarchy(const bpn::ndarray & obs, const bpn::ndarray & source, const bpn::ndarray & target, const bpn::ndarray & edge_weight,
                      const bpn::ndarray & lambda, const bpn::ndarray & cutoff, const int spatial, float weight_decay, bool verbose)
{//read data and run the L0-cut pursuit partition algorithm
    srand(0);
    float cp_verbose = verbose ? 1.f : 0.f;
    ...
    for (uint32_t ite_hierarchy = 0; ite_hierarchy < n_hierarchy; ite_hierarchy++)
    {
        if (spatial == 0)
        {
            CP::cut_pursuit<float>(n_ver, n_edg, n_obs, obs_data, source_data, target_data, edge_weight_data, &node_weight[0]
                     ...
                     , lambda_data[ite_hierarchy], (uint32_t)cutoff_data[ite_hierarchy],  1.f, 4.f, weight_decay, cp_verbose);
        }
        else
        {
            CP::cut_pursuit<float>(n_ver, n_edg, n_obs, obs_data, source_data, target_data, edge_weight_data, &node_weight[0]
                     , solution.data(), in_component, components
                     , n_nodes_red, n_edges_red, Eu_red, Ev_red, edgeWeight_red, nodeWeight_red
                     , lambda_data[ite_hierarchy], (uint32_t)cutoff_data[ite_hierarchy],  2.f, 4.f, weight_decay, cp_verbose);
        }
        n_ver = n_nodes_red;
        n_edg = n_edges_red;
...
BOOST_PYTHON_MODULE(libcp)
{
    ...
    def("cutpursuit", cutpursuit, (bp::args("cutoff")=0, bp::args("spatial")=0, bp::args("weight_decay")=1, bp::args("verbose")=false));
    def("cutpursuit_hierarchy", cutpursuit_hierarchy, (bp::args("cutoff")=0, bp::args("spatial")=0, bp::args("weight_decay")=1, bp::args("verbose")=false));
